  ...
  Future<void> sendPrayerRequest() async {
    ...
    try {
      ...
      if (targetImamId == null && selectedCategory != null) {
        ...
        if (categoryImams.docs.isNotEmpty) {
          // If only one imam handles this category, select them
          if (categoryImams.docs.length == 1) {
            targetImamId = categoryImams.docs.first['imamId'];
          } else {
            // Multiple imams - select the one with highest rating
            final imamRatings = await Future.wait(
              categoryImams.docs.map((doc) async {
                final ratingSnapshot = await FirebaseFirestore.instance
                    .collection('ratings')
                    .where('imamId', isEqualTo: doc['imamId'])
                    .get();

                double avgRating = 0;
                if (ratingSnapshot.docs.isNotEmpty) {
                  final total = ratingSnapshot.docs
                      .fold(0.0, (sum, doc) => sum + (doc['rating'] as num));
            ...
            // Sort by rating (highest first) and select the top imam
            imamRatings
                .sort((a, b) => b['avgRating'].compareTo(a['avgRating']));
            targetImamId = imamRatings.first['imamId'];
          }
          ...
          if (targetImamId != null) {
            final imamDoc = await FirebaseFirestore.instance
                ...
                .doc(targetImamId)
                .get();
            targetImamName = imamDoc['name'];
          }
        }
      }

      // If we still don't have an imam (no category selected or no imams for category)
      if (targetImamId == null) {
        // Select a random imam as fallback
        final random = Random();
        final randomImam = imams[random.nextInt(imams.length)];
        targetImamId = randomImam['id'];
        targetImamName = randomImam['name'];
      }

      // Schedule appointment if category is selected
      if (selectedCategory != null && targetImamId != null) {
        ...
        if (appointmentSnapshot.docs.isNotEmpty) {
          ...
          confirmationDay = appointmentData['day'];
          confirmationTime = appointmentData['time'];

          if (confirmationDay != null && confirmationTime != null) {
            final timeOfDay = parseTime(confirmationTime!);
            appointmentDate =
                getNextAppointmentDate(confirmationDay!, timeOfDay);
            confirmationMsg =
                "Your appointment has been scheduled with ${targetImamName ?? 'the Imam'}";
